...
function updateGameState(game: Game, db: Db) {
  return db.collection("games").updateOne({ _id: ObjectID.createFromHexString(game.id) }, {
...
function updatePlayer(player: Player, db: Db) {
  return db.collection("players").updateOne({ _id: ObjectID.createFromHexString(player.id) }, {
    hp: player.hp,
